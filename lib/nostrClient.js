...
`);
  ...
  async onNostrEvent(relay, sub_id, event ) {
    ...
    // TODO: do some input validation here, before storing stuff in the database
  ...
  async sendNostrDM(fromPubkey, fromPrivkey, toPubkey, obj, waitForAnswer = true, timeout = 30 ) {
    ...
    const event = await this.compileNostrDMEvent( fromPubkey, fromPrivkey, toPubkey, str );
  ...
  async sendNostrEphemeralDM(fromPubkey, fromPrivkey, toPubkey, obj, waitForAnswer = true, timeout = 30 ) {
    const str =  JSON.stringify(obj);
    const event = await this.compileNostrEphemeralDMEvent( fromPubkey, fromPrivkey, toPubkey, str );
  ...
  async compileNostrDMEvent( fromPubkey, fromPrivkey, toPubkey, message ) {
  ...
  async compileNostrEphemeralDMEvent(fromPubkey, fromPrivkey, toPubkey, message ) {
    return this.compileNostrEvent( {
      pubkey: fromPubkey,
