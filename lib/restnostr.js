...
module.exports = class Restnostr extends NostrClient {
  ...
  setupRoutes() {
    ...
    this.app.get('/keys', this.handleEndpoint );
    this.app.get('/keysets', this.handleEndpoint );
    this.app.get('/mint', this.handleEndpoint );
    this.app.post('/mint', this.handleEndpoint );
    this.app.post('/melt', this.handleEndpoint );
    this.app.post('/check', this.handleEndpoint );
    this.app.post('/checkfees', this.handleEndpoint );
    this.app.post('/split', this.handleEndpoint );
  ...
  async handleEndpoint( req, res ) {
    ...
    const jsonRPCBody = this.createJsonRPCBody( req.method.toLowerCase(), req.url, req.params, req.body );
    ...
    while( !timedOut ) {
      ...
      if( jsonRPCResponse ) {
        clearTimeout( rt );
        res.send( jsonRPCResponse.result );
        break;
      }
  ...
  createJsonRPCBody( method, endpoint, query, body ) {
    return {
      jsonrpc: '2.0',
      id: uuidv4(),
      method,
