...
module.exports = class Restnostr extends NostrClient {
  ...
  async handleEndpoint( req, res ) {
    ...
    const requestEventId = await this.sendToNostr( this.mintNostrPubkey, jsonRPCBody );
    let timedOut = false;
    let rt = setTimeout( () => {
      timedOut = true;
    ...
    while( !timedOut ) {
      ...
      if( entry ) {
        ...
        if( jsonRPCResultBody.result ) {
          ...
          await this.sendToNostr( this.mintNostrPubkey, this.createJsonRPCBody( 'GOTIT', {eventId: requestEventId} ) );
