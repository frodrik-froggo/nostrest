...
module.exports = class Nostrest extends NostrClient {
  ...
  async processGotit( pubkey, message ) {
    ...
    if( message.params && message.params.eventId ) {
      console.log( `DNE ${message.params.eventId}` );
  ...
  setupDatabase() {
    ...
    this.preparedStatements.cleanupWithResults = this.db.prepare('DELETE FROM events WHERE id IN ( SELECT r.id event_id FROM results r LEFT JOIN events e ON r.id = e.id WHERE r.status=2 AND e.status=0 ) AND created_at < strftime(\'%s\',\'now\', \'-5 minute\')');
  }
...
CREATE INDEX IF NOT EXISTS events_status_index ON results (status);
  ...
  async processEvent( event ) {
    ...
    console.log(`PRC ${event.id} -- ${event.pubkey}` );
  ...
  cleanup(nowSeconds ) {
    ...
    const r = this.preparedStatements.cleanupWithResults.run();
