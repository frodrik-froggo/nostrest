...
module.exports = class Nostrest extends NostrClient {
  ...
  async processHenlo( pubkey, message ) {
    // Answer with ITSME
    await this.sendToNostr( this.myPublicKey, this.myPrivateKey, pubkey, this.createJsonRPCResponse(message.id, {
      verb: 'ITSME',
  ...
  setupDatabase() {
    ...
    this.preparedStatements.cleanup = this.db.prepare('DELETE FROM events WHERE id IN ( SELECT r.id event_id FROM results r LEFT JOIN events e ON r.id = e.id WHERE r.status=2 AND e.status=0 )');
  }
...
CREATE INDEX IF NOT EXISTS events_status_index ON results (status);
  ...
  async processEvent( event ) {
    ...
    if( jsonRPCBody.method.toLowerCase() === 'henlo' ) {
      ...
      await this.deleteEvent( event.id); // TODO: move this to cleanup code, cause events can come in late and re-enter this into the database
      return 0;
    }
  ...
  async processResult( pubkey, jsonRPCResponse ) {
    ...
    await this.sendToNostr( this.myPublicKey, this.myPrivateKey, pubkey, jsonRPCResponse );
